	...
	for a := 1; a <= y; a++ {
		...
		for b := 1; b <= x; b++ {
			...
			if (a == 1 && b == 1) || (a == y && b == x) {
				...
				// vask alumine; parem ülemine
				// prindib '\'
				z01.PrintRune('\\')
			} else if b == 1 && (a != 1 || a != y) {
				// 1)esimese rea t2rn -  kui on esimene tulp aga mitte 1 v viimane koht, siis prindib '*'
				z01.PrintRune('*')
			} else if b == x && (a != y && a != 1) {
				// 1)viimase rea * -  kui on viimane tulp aga mitte 1 v viimane koht, siis prindib '*'
				z01.PrintRune('*')
			} else if a == 1 && (b != x && b != 1) {
