...
func QuadD(x, y int) {
	...
	for a := 1; a <= y; a++ {
		...
		for b := 1; b <= x; b++ {
			...
			if (a == 1 && b == 1) || (a == y && b == 1) {
				// 1) vasal ülemine; 2) vasak alumine;
				z01.PrintRune('A')
				// prindib 'A'
			} else if (b == x && a == 1) || (a == y && b == x) {
				// 1) parem 8lemine 2) parem alumine.
				z01.PrintRune('C')
			} else if b == 1 && (a != 1 || a != y) {
				// 1)esimese rea B -  kui on esimene tulp aga mitte 1 v viimane koht, siis prindib 'B'
				z01.PrintRune('B')
			} else if b == x && (a != y && a != 1) {
